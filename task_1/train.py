...
for i in range(32):
    ...
    filter_i = (filter_i - torch.min(filter_i)) / (torch.max(filter_i) - torch.min(filter_i))
    
    writer.add_image(f'conv1/weight/{i}', filter_i, dataformats='CHW')
...
output = model.model[0](img)
output = output.reshape(32, 112, 112)

for i in range(32):
    feature_map_i = output[i]
    
    feature_map_i = (feature_map_i - torch.min(feature_map_i)) / (torch.max(feature_map_i) - torch.min(feature_map_i))
    
    writer.add_image(f'conv1/output/{i}', feature_map_i, dataformats='HW')
